package com.bairuitech.demo; //[G,VideoConfigActivity]
...
public class VideoConfigActivity extends Activity{
	...
	private RadioGroup usageRadioGroup;		// 配置模式组
	
	RadioButton rbMaster;				// 服务器配置
	RadioButton rbSlave;				// 自定义配置
    ...
    {   
	 ...
	 // 选择用作Master还是Slave
    	...
    	usage.setText("本机将用作：");
    	mainLayout.addView(usage,new LayoutParams(LayoutParams.WRAP_CONTENT,LayoutParams.WRAP_CONTENT));
    	
       	usageRadioGroup = new RadioGroup(this);
    	rbMaster = new RadioButton(this);
    	rbSlave = new RadioButton(this);
       	rbMaster.setTextColor(Color.WHITE);
    	rbSlave.setTextColor(Color.WHITE);
    	rbMaster.setText("Master");
    	rbSlave.setText("Slave");
    	usageRadioGroup.addView(rbMaster);
    	usageRadioGroup.addView(rbSlave);
    	rbMaster.setOnClickListener(listener);
    	rbSlave.setOnClickListener(listener);

    	if(configEntity.usage == ConfigEntity.Master) //->[C]
    		rbMaster.setChecked(true);
    	else
    		rbSlave.setChecked(true);
    	
    	mainLayout.addView(usageRadioGroup,new LayoutParams(LayoutParams.WRAP_CONTENT,LayoutParams.WRAP_CONTENT));

	    
	    enableP2PBox = new CheckBox(this);
	    enableP2PBox.setText("启用P2P网络连接");
	    enableP2PBox.setTextColor(Color.WHITE);
	    ...
	    videoOverlayBox.setTextColor(Color.WHITE);
	    ...
	    videoRotateBox.setTextColor(Color.WHITE);
	    ...
	    fixColorDeviation.setTextColor(Color.WHITE);
	    ...
	    videoShowGPURender.setTextColor(Color.WHITE);
	    ...
	    videoAutoRotation.setTextColor(Color.WHITE);
	    ...
	    useARMv6Box.setTextColor(Color.WHITE);
	    ...
	    useAECBox.setTextColor(Color.WHITE);
	    ...
	    useHWCodecBox.setTextColor(Color.WHITE);
    	...
    	configModeLable.setTextColor(Color.WHITE);
    	...
    	configModeCustomBtn = new RadioButton(this);
       	configModeServerBtn.setTextColor(Color.WHITE);
    	configModeCustomBtn.setTextColor(Color.WHITE);
    	...
    	if(configEntity.configMode == ConfigEntity.VIDEO_MODE_SERVERCONFIG) //->[C]
    	...
    	resolutionLable.setTextColor(Color.WHITE);
    	...
    	{
    		Toast.makeText(getApplicationContext(), "G2.1", Toast.LENGTH_SHORT).show();
    		if(videoWidthValue[i] == configEntity.resolution_width)
    		{
    			Toast.makeText(getApplicationContext(), "G2.2", Toast.LENGTH_SHORT).show();
    			iSelectVideoSize = i;
    			break;
    		}
    	}
    ...
    {
    	Toast.makeText(this, "G3:" + bEnable, Toast.LENGTH_SHORT).show();
    ...
    {
    	Toast.makeText(this, "G4", Toast.LENGTH_SHORT).show();
    	configEntity.usage = rbMaster.isChecked()? ConfigEntity.Master : ConfigEntity.Slave;
    ...
    private Spinner InsertSpinnerInterface(String caption, String[] context, int[] value, int select) {
    	Toast.makeText(this, "G5", Toast.LENGTH_SHORT).show();
    	...
    	{
    		Toast.makeText(this, "G5.1", Toast.LENGTH_SHORT).show();
    		if(value[i] == select)
    		{
    			Toast.makeText(this, "G5.2", Toast.LENGTH_SHORT).show();
    			offset = i;
    			break;
    		}
    	}
